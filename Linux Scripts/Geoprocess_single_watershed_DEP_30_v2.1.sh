#!/usr/bin/env bash
###############################################################################################################
# Purpose: Calculate multiple terrain derivatives for a HUC6 format watershed 
#
...
USAGE="USAGE: $BASH_SOURCE -n [number of core] [HUC6 tile number]"
...
#The column name of the shapefiles attribute table with the HUC6 values.
fieldname="HUC6"
...
##############  1. Preprocessing  ##############
#**********Create subdirectories to hold derivatives result
working_dir=/scratch/summit/cskenny@xsede.org/Result/HUC6/
...
#Clip DEM to HUC6 watershed boundary.
echo now subsetting $fieldname $tile
gdalwarp --config GDAL_CACHEMAX 500 -wm 500 -multi -wo NUM_THREADS=$NumOfCore -t_srs "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0" -tr 30 30 -cutline $indexA -cwhere "$fieldname = '${tile}'" -crop_to_cutline -cblend $bufferA $DEM $working_dir${tile}/${tile}.tif
...
# 2. Trim off the edges of each derivative by a fraction of the original buffer to remove cells effected by edge artifacts
  function Trim_gdalwarp # $1: name of derivative
  {
    ...
    gdalwarp --config GDAL_CACHEMAX 500 -wm 500 -multi -wo NUM_THREADS=$NumOfCore -cutline $indexB -cwhere "$fieldname = '${tile}'" -cblend $bufferB -crop_to_cutline -tr 30 30 -r bilinear $working_dir${tile}/${tile}_${1}A.sdat $working_dir${tile}/${tile}_${1}.tif
  }
...
echo now Minimum Curvature of $fieldname ${tile}
Trim_gdalwarp mc
...
#Slope ##########
echo now calculating Slope of $fieldname ${tile}
$SAGA_parallel ta_morphometry 0 -ELEVATION=$working_dir${tile}/${tile}_s.sgrd -SLOPE=$working_dir${tile}/${tile}_slA.sgrd -METHOD=1 -UNIT_SLOPE=1
...
#Re-calculate slope in radians (this is fast so no reason not to calculate twice)
...
# Trim Catchment area (needed for twi and spi)
...
gdalwarp --config GDAL_CACHEMAX 500 -wm 500 -multi -wo NUM_THREADS=$NumOfCore -cutline $indexB -cwhere "$fieldname = '${tile}'" -cblend $bufferB -crop_to_cutline -tr 30 30  $working_dir${tile}/${tile}_s.sdat $working_dir${tile}/${tile}_s.tif
Delete_Temp_Files s
Compress _s
